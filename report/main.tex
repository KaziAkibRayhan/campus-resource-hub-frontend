...
% Landscape diagrams use a conservative printable area so that the
% illustration, frame, and caption remain clear of every page edge.
\newcommand{\reportwidefig}[4][0.78]{%
  \begin{figure}[p]
    \centering
    \IfFileExists{figures/#2}%
      {\fbox{\includegraphics[width=#1\linewidth]{figures/#2}}}%
      {\fbox{\parbox[c][5.5cm][c]{0.72\linewidth}{\centering
        \textit{Diagram unavailable}}}}
...
\begin{tikzpicture}[
  ...
  core/.style={draw=black,line width=0.8pt,rounded corners=3pt,
               align=center,font=\normalsize,minimum height=1.15cm,
               minimum width=4.1cm,fill=white},
  primary/.style={core,line width=1.2pt,minimum width=6.2cm},
  ...
  \node[core,below left=1.25cm and 0.45cm of api] (mongo)
       {\scriptsize\bfseries DATA TIER\\[2pt]
        \textbf{MongoDB Atlas}\\documents, indexes, vectors};
  \node[core,below right=1.25cm and 0.45cm of api] (cloud)
       {\scriptsize\bfseries DATA TIER\\[2pt]
        \textbf{Cloudinary}\\resource and image storage};

  \node[external,below=1.0cm of mongo] (brevo)
       {\scriptsize\bfseries EXTERNAL SERVICE\\[2pt]
        \textbf{Brevo SMTP}\\OTP and account e-mail};
  \node[external,below=1.0cm of cloud] (ai)
       {\scriptsize\bfseries AI SERVICES\\[2pt]
        \textbf{Provider Chain}\\Groq $\rightarrow$ Hugging Face $\rightarrow$ OpenAI};

  \draw[arr,<->] (react) -- node[lbl,right]{HTTPS / WSS} (api);
  \draw[arr,<->] (api.south) -- (mongo.north);
  \draw[arr,<->] (api.south) -- (cloud.north);
  \draw[arr] (api.west) -- ++(-3.25cm,0) |- (brevo.west);
  \draw[arr] (api.east) -- ++(3.25cm,0) |- (ai.east);

  \node[draw=black!60,rounded corners=5pt,inner sep=6pt,
        fit=(react)(api)(mongo)(cloud)(brevo)(ai)] (platform) {};
