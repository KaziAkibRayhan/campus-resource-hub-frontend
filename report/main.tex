...
\definecolor{crhblue}{HTML}{000000}
\definecolor{crhgold}{HTML}{000000}
...
\newcommand{\reportfig}[4][1.0]{%
  \begin{figure}[htbp]
    ...
    \IfFileExists{figures/#2}%
      {\fbox{\includegraphics[width=#1\linewidth]{figures/#2}}}%
      {\fbox{\parbox[c][5.5cm][c]{0.8\linewidth}{\centering
...
\begin{titlepage}
  ...
  \begin{center}
    ...
    {\color{crhblue}\LARGE\bfseries Campus Resource Hub\par}
    \vspace{0.35cm}
    {\Large\bfseries A Centralized Web Platform for Academic\\[5pt]
      Resource Sharing with AI-Powered Moderation\par}
    \vspace{0.65cm}
    {\large\itshape A secure and intelligent platform for academic\\
      resources and campus collaboration\par}
    \vspace{1.25cm}
    ...
    \IfFileExists{figures/university-logo.png}%
      {\includegraphics[width=3.15cm]{figures/university-logo.png}}%
      {\fbox{\parbox[c][3.5cm][c]{4cm}{\centering\textit{University\\logo}}}}
    \vspace{1.05cm}

    {\color{crhblue}\large\bfseries
      DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING\par}
    \vspace{0.18cm}
    {\large\bfseries NORTHERN UNIVERSITY BANGLADESH\par}
    \vspace{0.55cm}
    {\color{crhgold}\rule{3.2cm}{0.7pt}\par}
    \vspace{0.28cm}
...
First and foremost, we express our sincere gratitude to Almighty God,
the Most Merciful, for granting us the strength, patience, and
opportunity to complete this project successfully. The development of
\textit{Campus Resource Hub} was a demanding but rewarding journey,
and its completion would not have been possible without the guidance,
...
Science and Engineering, Northern University Bangladesh. His
thoughtful guidance, constructive criticism, and continuous
encouragement helped us refine both the technical implementation and
the presentation of this work. His willingness to review our ideas,
identify weaknesses, and suggest practical improvements was
invaluable throughout the planning, development, testing, and report
preparation stages.

We are also sincerely grateful to \DeptHeadName, Associate Professor
\& Head of the Department of Computer Science \& Engineering,
Northern University Bangladesh, for providing the academic direction
and supportive environment necessary to carry out this project. We
extend our thanks to all respected faculty members of the department
whose courses, advice, and classroom instruction gave us the
knowledge required to work with software engineering, database
systems, web technologies, networking, information security, and
applied artificial intelligence. We equally appreciate the
administrative and laboratory staff for their cooperation and
assistance during our undergraduate studies.

We gratefully acknowledge our classmates, friends, and the students
who shared practical observations about resource exchange, campus
communication, and usability. Their feedback during informal testing
helped us recognize real user needs and improve several workflows of
the system. We also thank the developers and maintainers of the
open-source technologies and documentation used in this project,
which made it possible for us to design and implement a full-stack
platform within a student budget.

Finally, we offer our heartfelt gratitude to our parents and family
members for their unconditional support, patience, prayers, and
motivation throughout our education. We also appreciate one another
as team members for sharing responsibilities, resolving challenges,
and remaining committed through every stage of the project. We
dedicate this work to everyone who supported our academic journey and
inspired us to create a system that may contribute meaningfully to
...
\section{Limitations Identified in Previous Work}
...
\begin{tikzpicture}[
  ...
  core/.style={draw=black,line width=0.8pt,rounded corners=3pt,
               align=center,font=\small,minimum height=1.05cm,
               minimum width=3.7cm,fill=white},
  primary/.style={core,line width=1.2pt,minimum width=5.2cm},
  external/.style={core,dashed,minimum width=3.1cm,fill=black!3},
  ...
  \node[primary,below=1.2cm of react] (api)
       {\scriptsize\bfseries APPLICATION TIER\\[2pt]
        \textbf{Node.js / Express REST API}\\Socket.io gateway on Render};

  \node[core,below left=1.2cm and -0.25cm of api] (mongo)
       {\scriptsize\bfseries DATA TIER\\[2pt]
        \textbf{MongoDB Atlas}\\documents, indexes, vectors};
  \node[core,below right=1.2cm and -0.25cm of api] (cloud)
       {\scriptsize\bfseries DATA TIER\\[2pt]
        \textbf{Cloudinary}\\resource and image storage};

  \node[external,below=1.05cm of mongo] (brevo)
       {\textbf{Brevo SMTP}\\OTP and account e-mail};
  \node[external,below=1.05cm of cloud] (ai)
       {\textbf{AI Provider Chain}\\Groq $\rightarrow$ Hugging Face $\rightarrow$ OpenAI};

  \draw[arr,<->] (react) -- node[lbl,right]{HTTPS / WSS} (api);
  \draw[arr,<->] (api) -- node[lbl,left,pos=.62]{Mongoose} (mongo);
  \draw[arr,<->] (api) -- node[lbl,right,pos=.62]{signed media access} (cloud);
  \draw[arr] (api.west) -- ++(-2.55cm,0) |-
    node[lbl,left,pos=.72]{transactional e-mail} (brevo.west);
  \draw[arr] (api.east) -- ++(2.55cm,0) |-
    node[lbl,right,pos=.72]{moderation and answers} (ai.east);

  \node[draw=black!60,rounded corners=5pt,inner sep=6pt,
        fit=(react)(api)(mongo)(cloud)] (platform) {};
  \node[font=\scriptsize\bfseries,fill=white,inner sep=2pt,
        above left=-1mm and 3mm of platform.north west]
        {CORE CAMPUS RESOURCE HUB};
\end{tikzpicture}
}
\caption{Monochrome three-tier system architecture and external service boundaries}
\label{fig:arch}
\end{figure}
...
\begin{landscape}
\reportfig[0.80]{workflow-diagram.pdf}{End-to-end user access,
resource moderation, publication, campus-service, and notification
workflow}{fig:workflow}
\end{landscape}
...
\begin{landscape}
\reportfig[0.80]{erd-diagram.pdf}{MongoDB collection relationships
supporting authorization, academic content, communication, and AI
retrieval}{fig:erd}
\end{landscape}
