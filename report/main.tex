...
\newcommand{\reportwidefig}[4][0.70]{%
  \begin{figure}[p]
    \centering
    \IfFileExists{figures/#2}%
      {\fbox{\includegraphics[width=#1\linewidth]{figures/#2}}}%
...
\begin{tikzpicture}[
  ...
  core/.style={draw=black,line width=0.8pt,rounded corners=3pt,
               ...
               minimum width=4.3cm,fill=white},
  ...
  \node[core,below left=1.35cm and 0.1cm of api] (data)
  ...
  \node[external,below=1.35cm of api] (mail)
       {\scriptsize\bfseries COMMUNICATION SERVICE\\[2pt]
        \textbf{Brevo SMTP}\\OTP and account e-mail};
  \node[external,below right=1.35cm and 0.1cm of api] (ai)
       {\scriptsize\bfseries AI SERVICES\\[2pt]
        \textbf{Moderation and Assistance}\\Groq $\rightarrow$ Hugging Face\\$\rightarrow$ OpenAI};

  \draw[arr,<->] (react) -- node[lbl,right]{HTTPS / WSS} (api);
  \draw[arr,<->] (api.south) -- (data.north);
  \draw[arr] (api.south) -- (mail.north);
  \draw[arr] (api.south) -- (ai.north);
