...
import { otpSchema, signupSchema } from "../../utils/validationSchemas";
...
const Signup = () => {
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    if (result.success) {
      ...
      setSignupMessage(result.message || "Verification code sent to your email");
    } else {
  ...
  const handleVerifyOtp = async (values, { setSubmitting }) => {
    setSignupError("");
    const result = await verifySignupOtp(pendingEmail, values.otp);

    if (result.success) {
      navigate("/dashboard");
    } else {
      setSignupError(result.error || "Verification failed. Please try again.");
    }
          ...
          {signupMessage && (
            <div className="mb-6 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg flex items-start space-x-3">
              <CheckCircle
                ...
                size={20}
              />
              <p className="text-green-700 dark:text-green-400 text-sm">{signupMessage}</p>
            </div>
          )}
              ...
              {({ isSubmitting, errors, touched }) => (
                <Form className="space-y-6">
                  ...
                  <button
                    ...
                    className="w-full text-sm font-semibold text-blue-600 hover:text-blue-500 dark:text-blue-400"
                  >
                    Edit signup details
                  </button>
                </Form>
